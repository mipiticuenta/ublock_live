...
while n_1 > len(list2) :                                                         # <recursive loops until steady list2 length />
    ...
    list5 = [
        ...
        if not re.search(r'^/\*', re.sub(r'\$important$', '', line))                                         # <remove /* bad regex filter />
        ...
        if re.sub(r'\$important$', '', line)[1: -1] not in iana_sld                                          # <remove /item/ if item in aiana_sld />
        if len(re.sub(r'\$important$', '', line)) > 5                                                        # <remove too short regex filter />
    ]
